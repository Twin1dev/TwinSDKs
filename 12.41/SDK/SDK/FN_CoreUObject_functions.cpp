...
namespace SDK
{
	...
	//class UFunction* UClass::GetFunction(const std::string& ClassName, const std::string& FuncName)
	//{
	//	static auto StaticFindObject = (UFunction * (*)(UClass*, UObject*, const wchar_t* OrigInName, bool ExactClass))((uintptr_t)GetModuleHandleA(0) + 0x2e1c4b0);
	//	static auto StaticLoadObject = (UFunction * (*)(UClass*, UObject*, const wchar_t* InName, const wchar_t* Filename, uint32_t LoadFlags, UObject * Sandbox, bool bAllowObjectReconciliation))((uintptr_t)GetModuleHandle(0) + 0x2e1d7a0);

	//	// fucking sucks
	//	std::string kys = ClassName;
	//	std::string kys2 = FuncName;



	//	std::string ObjectName = "/Script/" + kys + "." + kys2;
	//	

	//	auto Function = StaticFindObject(UFunction::StaticClass(), nullptr, std::wstring(ObjectName.begin(), ObjectName.end()).c_str(), false);

	//	if (!Function)
	//	{
	//		std::string ObjectNameEngine = "/Script/Engine." + kys + "." + kys2;
	//		auto Function = StaticFindObject(UFunction::StaticClass(), nullptr, std::wstring(ObjectNameEngine.begin(), ObjectNameEngine.end()).c_str(), false);

	//		if (!Function)
	//		{
	//			std::string ObjectNameFN = "/Script/FortniteGame." + kys + "." + kys2;
	//			auto Function = StaticFindObject(UFunction::StaticClass(), nullptr, std::wstring(ObjectNameFN.begin(), ObjectNameFN.end()).c_str(), false);

	//			if (!Function)
	//			{
	//				std::cout << "Slowly Finding " << ObjectName << "\n";
	//				for (UStruct* Clss = this; Clss; Clss = Clss->Super)
	//				{
	//					if (Clss->GetName() == ClassName)
	//					{
